...
# ==================== 改进版DAMBlock (带自适应深度和跳跃连接) ====================
class DAMBlockV2(nn.Module):
    """
    改进的DAMBlock

    改进点:
    1. 可变深度机制: 根据尺度自适应调整处理深度
    2. 跳跃连接: 增强梯度流
    3. 门控残差融合: 替代简单stack
    """
    def __init__(self, base_dim=64, out_d=[64, 128, 256, 256], use_depthwise=True, use_skip=True):
        ...
        if use_skip:
            self.skip_weights = nn.ParameterList([
                nn.Parameter(torch.tensor(0.2)),  # d2
                nn.Parameter(torch.tensor(0.25)), # d3
                nn.Parameter(torch.tensor(0.3)),  # d4
                nn.Parameter(torch.tensor(0.35))  # d5
            ])
...
class DEAM_with_Swsi(nn.Module):
    ...
    def forward(self, input, diff, s_wsi=None):
        # Adaptive pooling to handle small feature maps
        # For very small maps (e.g., 4x4), reduce ds to avoid zero-sized output
        _, _, h_orig, w_orig = input.size()
        ds_adaptive = min(self.ds, max(1, h_orig // 2))  # Ensure output is at least 2x2

        if ds_adaptive < self.ds:
            pool_adaptive = nn.AvgPool2d(ds_adaptive)
            x = pool_adaptive(input)
            diff = pool_adaptive(diff)
        else:
            x = self.pool(input)
            diff = self.pool(diff)
        ...
        if s_wsi is not None:
            ...
            sim_sigmoid = torch.sigmoid(s_wsi_resized)

            # 广播加法: 增强相似区域的注意力响应
            sim_flat = sim_sigmoid.view(b, 1, h * w)
        ...
        # 双重增强: Value 调制 (保留)
        if s_wsi is not None:
            # 复用已计算的sim_sigmoid，避免重复插值
            sim_gate = sim_sigmoid.view(b, -1, h * w)
            proj_value = proj_value * (1.0 + 0.1 * sim_gate)

        out = torch.bmm(proj_value, attention.permute(0, 2, 1))
        out = out.view(b, self.input_dim, h, w)
        out = F.interpolate(out, [h * ds_adaptive, w * ds_adaptive], mode='bilinear', align_corners=True)
...
# ==================== 改进版DEAM (多头注意力增强) ====================
class DEAM_with_Swsi_V2(nn.Module):
    """
    改进的双时相增强注意力模块

    改进点:
    1. 自适应下采样因子: 根据特征图大小动态调整
    2. 多头注意力: 增强特征表达能力
    3. 不确定性建模: 可选的MC Dropout
    """

    def __init__(self, input_dim, diff_dim, ds=8, beta_init=0.3, n_heads=4, use_uncertainty=False):
        super(DEAM_with_Swsi_V2, self).__init__()
        self.input_dim = input_dim
        self.diff_dim = diff_dim
        self.ds = ds
        ...
        # 多头注意力配置
        self.head_dim = diff_dim // (n_heads * 8)  # 每个头的维度
    ...
    def _multi_head_attention(self, query, key, value, sim_bias=None):
        ...
        # 分头
        query = query.view(b, self.n_heads, self.head_dim, h * w).permute(0, 1, 3, 2)  # [B, heads, HW, head_dim]
        key = key.view(b, self.n_heads, self.head_dim, h * w)  # [B, heads, head_dim, HW]

        # 计算注意力
        energy = torch.matmul(query, key)  # [B, heads, HW, HW]
        ...
        attention = self.softmax(energy)  # [B, heads, HW, HW]

        # 应用注意力到value
        value_heads = []
        for i in range(self.n_heads):
            attn_i = attention[:, i, :, :]  # [B, HW, HW]
            value_flat = value.view(b, c, h * w)  # [B, C, HW]
            out_i = torch.bmm(value_flat, attn_i.permute(0, 2, 1))  # [B, C, HW]
            value_heads.append(out_i.view(b, c, h, w))

        # 融合多头
        multi_head_out = torch.cat(value_heads, dim=1)  # [B, C*heads, H, W]
        fused_out = self.multi_head_fusion(multi_head_out)  # [B, C, H, W]
    ...
    def forward(self, input, diff, s_wsi=None):
        ...
        b, c, h, w = diff_down.size()

        # 计算query和key
        proj_query = self.query_conv(diff_down)  # [B, head_dim*heads, H, W]
        ...
        if s_wsi is not None:
            s_wsi_resized = F.interpolate(s_wsi, size=(h, w), mode='bilinear', align_corners=True)
            sim_sigmoid = torch.sigmoid(s_wsi_resized)
            sim_bias = sim_sigmoid.view(b, -1, h * w)
        ...
        if self.use_uncertainty and not self.training:
            out, _ = self.uncertainty(out, return_uncertainty=True)
        elif self.use_uncertainty:
            out = self.uncertainty(out, return_uncertainty=False)
